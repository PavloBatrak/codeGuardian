...
function preload() {
    this.load.image('background', '/static/assets/advanced-office-background.png');
    this.load.image('ground', '/static/assets/ground.png');
    this.load.image('bug', '/static/assets/bug.png');
    this.load.image('script', '/static/assets/script.png'); // Symbolizuje skrypt automatyzacji
...
function create() {
    ...
    // Skrypty do poprawienia
    scripts = this.physics.add.group({
        key: 'script',
        repeat: 2,
        setXY: { x: 150, y: 100, stepX: 300 }
    });

    // Kolizje
    this.physics.add.collider(player, platforms);
    this.physics.add.collider(bugs, platforms);
    this.physics.add.collider(scripts, platforms);

    // Zbieranie błędów i skryptów
    this.physics.add.overlap(player, bugs, collectBug, null, this);
    this.physics.add.overlap(player, scripts, fixScript, null, this);

    // Tekst licznika błędów i skryptów
    bugText = this.add.text(16, 16, 'Błędy: 0/10', { fontSize: '32px', fill: '#000' });
    scriptText = this.add.text(16, 50, 'Naprawione skrypty: 0/3', { fontSize: '32px', fill: '#000' });
...
function collectBug(player, bug) {
    ...
    if (collectedBugs === 10 && fixedScripts === 3) {
        this.add.text(200, 300, 'Świetna robota! Automatyzacja pomogła w testach!',
                      { fontSize: '32px', fill: '#00FF00' });
        gameOver = true;
    }
}

function fixScript(player, script) {
    script.disableBody(true, true);
    fixedScripts += 1;
    scriptText.setText('Naprawione skrypty: ' + fixedScripts + '/3');
